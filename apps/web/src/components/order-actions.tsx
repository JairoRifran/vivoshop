...
import { cancelOrder, payOrder } from '@/lib/actions/checkout';

/**
 * Buyer-side order actions.
 *
 * Which buttons exist is derived from the order status rather than always
 * rendered and disabled — a greyed-out "Cancelar" on a delivered order is
...
export function OrderActions({
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {canPay ? (
        <Button block loading={pending} onClick={() => run(() => payOrder(orderId))}>
          Pagar ahora
        </Button>
      ) : null}
